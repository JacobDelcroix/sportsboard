...
The full editor provides its own keyboard and touch interaction layer. Copy, cut, paste, delete, undo, and redo apply to the current selection. Keyboard commands are ignored while the user is typing in the number field or notes. Pasted elements receive a small offset and a new ID; attached items and connector endpoints are pasted as independent, movable geometry.
